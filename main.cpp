...
int main() {
    ...
    bool seguir = true;
    bool irAlMenu = true; // Controla si se muestra el menu o se va directo a nueva compra

    while (seguir) {

        if (irAlMenu) {
            seguir = caja.iniciar_compra();
            if (!seguir) break; // Sale si el usuario eligio 0
        }
        irAlMenu = true; // Por defecto, la proxima vez si muestra el menu
        ...
        for (int i = 0; i < boletos_cantidad; i++) {
            Boleto* b = new Boleto(
                ...
                asientos_elegidos[i],
                precio_unitario,
                (tipo_cli == 1? "Regular" : "VIP"),
                seleccion
            );
            b->generar_boleto();
            caja.registrarBoleto(b); //Se guarda en el registro de la caja
        }
        ...
        caja.registrarCliente(cliente); //Se guarda en el registro de la caja

        //Menu post-compra
        int opFinal;
        cout << "Que desea hacer ahora?" << endl;
        cout << "1. Regresar al menu principal" << endl;
        cout << "2. Realizar una nueva compra" << endl;
        cout << "Opcion: ";
        cin >> opFinal;
        cin.ignore();
        cout << endl;
        ...
        if (opFinal == 2) {
            irAlMenu = false; // Salta el menu y va directo a nueva compra
        }
        // Si elige 1, irAlMenu queda true y el menu aparece
    }

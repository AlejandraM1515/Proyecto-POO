...
Caja::Caja() {
    totalClientes = 0;
    totalBoletos  = 0;
    for (int i = 0; i < 100; i++) { //Recorre las 100 posiciones de ambos arreglos y pone lso punteros en vacio para leugo ser rellenados
        clientes[i] = nullptr;
        boletos[i]  = nullptr;
    }
}
...
//otros metodos
void Caja::registrarCliente(Cliente* c) { //Recibe un puntero a cliente para luego guardarlo en el arreglo de clientes
    if (totalClientes < 100)
        clientes[totalClientes++] = c;
}

void Caja::registrarBoleto(Boleto* b) { //Recibe un puntero a boleto para luego guardarlo en el arreglo de boletos
